...
func TimeoutTests(f *framework.Framework) {
	...
	Describe("Deploy Item Timeouts", func() {
		...
		It("should detect timeouts", func() {
			...
			Expect(metav1.HasAnnotation(dummy_inst_di.ObjectMeta, string(lsv1alpha1helper.ReconcileTimestamp))).To(BeTrue(), "dummy deploy item should have a reconcile timestamp annotation")
			...
			Eventually(func() lsv1alpha1.DeployItem { // check mock_di_prog first, because it's the only one that will change again (aborting timeout)
				...
				"Status": MatchFields(IgnoreExtras, Fields{
					"Phase": Equal(lsv1alpha1.ExecutionPhaseProgressing),
				}),
			}))
			...
			Eventually(func() lsv1alpha1.DeployItem {
				...
				"ObjectMeta": MatchFields(IgnoreExtras, Fields{
					"Annotations": Not(MatchKeys(IgnoreExtras, Keys{
						lsv1alpha1.ReconcileTimestampAnnotation: Not(BeNil()),
					})),
				}),
				"Status": MatchFields(IgnoreExtras, Fields{
					"Phase": Equal(lsv1alpha1.ExecutionPhaseSucceeded),
				}),
