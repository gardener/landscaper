...
func SimpleImportForNewReconcile(f *framework.Framework) {
	_ = Describe("SimpleImport", func() {
		...
		It("should deploy a nginx ingress controller and a echo-server", func() {
			var (
				nginxTutorialResourcesRootDir      = filepath.Join(f.RootPath, "/test/integration/tutorial/resources/local-ingress-nginx")
				echoServerTutorialResourcesRootDir = filepath.Join(f.RootPath, "/test/integration/tutorial/resources/echo-server")
