...
1. In the [settings](commands/settings) file, adjust the variable `TARGET_CLUSTER_KUBECONFIG_PATH` so that it contains
...
It applies a [target.yaml](installation/target.yaml.tpl) and an [installation.yaml](installation/installation.yaml.tpl)
on the resource cluster.
