...
if [[ $LOCAL_BLUEPRINT == "local" ]]; then
    ...
    ocm add resource ${COMPONENT_DIR} --type blueprint --name blueprint --version 1.0.0 --accessType ociArtifact \
        --reference eu.gcr.io/gardener-project/landscaper/examples/blueprints/guided-tour/helm-chart-resource:1.0.0
fi
...
ocm add resource ${COMPONENT_DIR} --type helmChart --name hello-world-chart --version 1.0.0 --accessType ociArtifact \
    --reference eu.gcr.io/gardener-project/landscaper/examples/charts/hello-world:1.0.0
