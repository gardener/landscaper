	...
	Context("with clock", func() {
		...
		BeforeEach(func() {
			...
			ctrl = installationsctl.NewTestActuator(testenv.Client, testenv.Client, testenv.Client, *op,
				logging.Discard(), clok, &config.LandscaperConfiguration{}, "test-inst2-"+testutils.GetNextCounter())
		})
		...
		It("should retry a failed installation with cron spec", func() {
			ctx := context.Background()

			var err error
			state, err = testenv.InitResources(ctx, "./testdata/state/test11")
			Expect(err).ToNot(HaveOccurred())
			Expect(testutils.CreateExampleDefaultContext(ctx, testenv.Client, state.Namespace)).To(Succeed())
