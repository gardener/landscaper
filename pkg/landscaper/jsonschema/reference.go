	...
	case map[string]interface{}:
		...
		for k, v := range typedData {
			...
			if k == "$ref" {
				...
				uri, ok := v.(string)
				if !ok {
					return nil, fmt.Errorf("invalid reference value: expected string, got %v", v)
				}
				if len(typedData) > 1 {
					return nil, fmt.Errorf("invalid reference: fields next to '$ref' are not allowed")
				}
				sub, err := rr.resolveReference(uri, subPath)
				if err != nil {
					return nil, fmt.Errorf("error resolving reference at %s: %w", subPath, err)
				}
				res = sub
				break
			}
		...
		return resMap, nil
	case []interface{}:
		resList := make([]interface{}, len(data.([]interface{})))
		for i, e := range data.([]interface{}) {
