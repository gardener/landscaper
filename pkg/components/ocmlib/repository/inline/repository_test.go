...
var _ = XDescribe("ocm-lib based landscaper local repository", func() {
	...
	BeforeEach(func() {
		env = builder.NewBuilder(tenv.NewEnvironment(tenv.TestData()))
	})
	...
	It("repository spec with inline component descriptors and local resources", func() {
		...
		Must(env.OCMContext().RepositorySpecForConfig(specdata, runtime.DefaultYAMLEncoding))
		// TODO
	...
	It("repository with a directory resource", func() {
		// TODO
		// Must(inline.NewRepositorySpecV1(env, DIRECTORY_REPOSITORY, nil, DIRECTORY_REPOSITORY))
		//repo := Must(spec.Repository(env.OCMContext(), nil))
		//defer Close(repo)
		//cv := Must(repo.LookupComponentVersion(COMPONENT_NAME, COMPONENT_VERSION))
		//defer Close(cv)
