...
func NewCache(log logr.Logger, options ...Option) (*layeredCache, error) {
	...
	//initialize metrics
	baseCFs.WithMetrics(
		CachedItems.WithLabelValues(opts.UID),
		CacheDiskUsage.WithLabelValues(opts.UID),
		CacheHitsDisk.WithLabelValues(opts.UID))
	if opts.InMemoryOverlay {
		overlayCFs.WithMetrics(nil,
			CacheMemoryUsage.WithLabelValues(opts.UID),
			CacheHitsMemory.WithLabelValues(opts.UID))
	}
