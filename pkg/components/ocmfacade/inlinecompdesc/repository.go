...
func NewRepository(ctx cpi.ContextProvider, descriptorFs vfs.FileSystem, descriptorPath string, blobFs vfs.FileSystem, blobPath string) (cpi.Repository, error) {
	var err error
	if descriptorPath != "" && descriptorPath != "/" {
		descriptorFs, err = projectionfs.New(descriptorFs, descriptorPath)
		if err != nil {
			return nil, err
		}
	}

	if blobPath != "nil" && blobPath != "/" {
		blobFs, err = projectionfs.New(blobFs, blobPath)
		if err != nil {
			return nil, err
		}
	}

	acc, err := NewAccess(descriptorFs, blobFs)
	if err != nil {
		return nil, err
	}
