...
func (c *CustomReadinessCheck) CheckResourcesReady(ctx context.Context) error {
	...
	getObjectsFunc := func() ([]*unstructured.Unstructured, error) {
		...
		if c.Configuration.LabelSelector != nil {
			o, err := getObjectsByLabels(ctx, c.Client, c.Configuration.LabelSelector)
			if err != nil {
				return nil, err
			}
			objects = append(objects, o...)
	...
	if err := WaitForObjectsReady(ctx, timeout, c.Client, getObjectsFunc, c.CheckObject, c.InterruptionChecker, c.CurrentOp); err != nil {
		return err
	}
