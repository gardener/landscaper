...
func (d *deployer) Reconcile(ctx context.Context, lsCtx *lsv1alpha1.Context, di *lsv1alpha1.DeployItem, _ *lsv1alpha1.Target) error {
	...
	return read_write_layer.UpdateDeployItemStatus(ctx, d.lsClient.Status(), di)
}
...
func (d *deployer) ensureExport(ctx context.Context, item *lsv1alpha1.DeployItem, config *mockv1alpha1.ProviderConfiguration) error {
	...
	return read_write_layer.UpdateDeployItemStatus(ctx, d.lsClient.Status(), item)
}
...
func (d *deployer) getConfig(ctx context.Context, item *lsv1alpha1.DeployItem) (*mockv1alpha1.ProviderConfiguration, error) {
	...
	if _, _, err := Decoder.Decode(item.Spec.Configuration.Raw, nil, config); err != nil {
		...
		_ = read_write_layer.UpdateDeployItemStatus(ctx, d.lsClient.Status(), item)
		return nil, err
	}
