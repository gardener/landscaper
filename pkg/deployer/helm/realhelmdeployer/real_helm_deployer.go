...
func (c *RealHelmDeployer) installRelease(ctx context.Context, values map[string]interface{}) (*release.Release, error) {
	...
	c.log.Logr().V(1).Info(fmt.Sprintf("installing release %s into namespace %s", c.releaseName, c.defaultNamespace))
	...
	c.log.Logr().V(1).Info(fmt.Sprintf("installing helm chart release %s", c.releaseName))
	...
	c.log.Logr().V(1).Info(fmt.Sprintf("%s successfully installed in %s", c.releaseName, c.defaultNamespace))
...
func (c *RealHelmDeployer) upgradeRelease(ctx context.Context, values map[string]interface{}) (*release.Release, error) {
	...
	c.log.Logr().V(1).Info(fmt.Sprintf("upgrading release %s", c.releaseName))
	...
	c.log.Logr().V(1).Info(fmt.Sprintf("upgrading helm chart release %s", c.releaseName))
	...
	c.log.Logr().V(1).Info(fmt.Sprintf("%s successfully upgraded in %s", c.releaseName, c.defaultNamespace))
...
func (c *RealHelmDeployer) deleteRelease(ctx context.Context) error {
	...
	c.log.Logr().V(1).Info(fmt.Sprintf("deleting release %s in namespace %s", c.releaseName, c.defaultNamespace))
	...
	c.log.Logr().V(1).Info(fmt.Sprintf("%s successfully deleted in %s", c.releaseName, c.defaultNamespace))
...
func (c *RealHelmDeployer) createLogFunc(_ context.Context) func(format string, v ...interface{}) {
	return func(format string, v ...interface{}) {
		c.log.Logr().V(1).Info(fmt.Sprintf(format, v))
	}
}
